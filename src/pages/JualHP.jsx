...
const JualHP = () => {
  ...
  const calculatePrice = (model, storage) => {
      ...
      else if (model.includes("iPhone 14 Pro Max")) { min = 14.5; max = 16.5; img = "/13 result.jpg"; } // Fallback
      ...
      else if (model.includes("Samsung")) { min = 5.0; max = 10.0; } // Generic Samsung range
      else { min = 2.0; max = 5.0; }

      // Storage adjustment (Rough estimate: +500k for higher storage)
  ...
  const handleCalculate = (data) => {
      const { range, image } = calculatePrice(data.model, data.storage);
      setResultData({
          ...data,
          priceRange: range,
          image: image
  ...
  return (
    <div className="pt-24 pb-12 min-h-screen bg-gray-50 max-w-7xl mx-auto px-4">
      {resultData ? (
          <PriceResult result={resultData} onBack={handleBack} />
      ) : (
          <>
